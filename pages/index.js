...
const Index = props => {
  ...
  function handleToggleInterested(name) {
    setInterested(interested => ({ ...interested, [name]: interested[name] ? false : true }));
  }
    ...
    <div>
      ...
      {sessionList.map(session => (
        <div key={session}>
          <h1>{session}</h1>
          <div>
            {props.sessions[session].map(beer => (
              <Beer
                key={beer.name}
                beer={beer}
...
Index.getInitialProps = async () => {
  ...
  const sessions = beers.reduce((sessions, beer) => {
    if (!sessions[beer.session]) sessions[beer.session] = [];
    sessions[beer.session].push(beer);
    return sessions;
  });
  return { sessions };
};
